...
// v0.44: bilingual room labels
const ROOM_I18N_EN={
  house:{name:"Merchant's house",desc:"The heart of the trading house: here the hero lives, receives kin and gradually restores the family hearth.",action:"Prepare rations",effect:"A restored kitchen with a cook lowers upkeep and brings small income.",stat:"Comfort",job:"Kitchen"},
  warehouse:{name:"Warehouse",desc:"A protected room for trade stocks. Opening the warehouse greatly expands inventory.",action:"Tighten security",effect:"Storekeepers grow capacity; guards reduce losses.",stat:"Capacity",job:"Warehouse"},
  forge:{name:"Forge",desc:"A workshop for metalwork. Only an assigned blacksmith can craft tools, weapons and armour.",action:"Forge tools",effect:"The smith's craft and equipment shape income and quality.",stat:"Craft",job:"Forge"},
  weaving:{name:"Weaving shop",desc:"A workshop for textiles and leather. The weaver makes carpets, clothing, bags and caps.",action:"Weave a carpet",effect:"Without a weaver the shop falls silent; the loom and craft skill raise profit.",stat:"Craft",job:"Weaving shop"},
  cells:{name:"Holding cells",desc:"A closed space for NPCs the owner deems dangerous or unruly.",action:"Conduct an inspection",effect:"Those held grow obedient but lose loyalty.",stat:"Control",job:"Cell"},
  stable:{name:"Stable",desc:"A place for horses, harness and caravan prep. The hostler's strength helps caravans.",action:"Prepare the horses",effect:"A stable worker brings income and lowers road risk.",stat:"Speed",job:"Stable"},
  inn:{name:"Tavern",desc:"A hall for guests and travellers. Later you'll gather rumours and orders here.",action:"Hold a feast",effect:"A free worker with hospitality turns guests into profit.",stat:"Hospitality",job:"Inn"},
  training:{name:"Training hall",desc:"A yard for weapon drills and preparing the trading house's guards.",action:"Run a drill",effect:"Combat skill grows and helps defend caravans.",stat:"Drill",job:"Training"},
  jewelry:{name:"Jeweller's workshop",desc:"A workshop for fine metals and stones.",action:"Craft jewellery",effect:"A jeweller turns silver and glass into prized adornments.",stat:"Craft",job:"Jeweller's workshop"},
  furniture:{name:"Carpenter's workshop",desc:"A workshop for furniture and fittings.",action:"Build furniture",effect:"A carpenter turns tools and leather into furnishings.",stat:"Craft",job:"Carpenter's workshop"}
};
function roomI18n(r,field){const en=ROOM_I18N_EN[r.key]&&ROOM_I18N_EN[r.key][field];return lang==="en"&&en?en:r[field];}
function roomName(r){return roomI18n(r,"name");}
function roomDesc(r){return roomI18n(r,"desc");}
function roomAction(r){return roomI18n(r,"action");}
function roomEffect(r){return roomI18n(r,"effect");}
function roomStat(r){return roomI18n(r,"stat");}
function roomJob(r){return roomI18n(r,"job");}
...
function renderRooms(){
  ...
  document.getElementById("roomTabs").innerHTML=rooms.map((r,i)=>`<button class="room-tab ${i===currentRoom?"active":""} ${r.unlocked?"":"locked"}" onclick="selectRoom(${i})">${r.icon} ${roomName(r)}${r.unlocked?"":" 🔒"}</button>`).join("");
  const r=rooms[currentRoom];
  const remoteNotice=!atHeadquarters()?`<div class="economy-note">${tr("Штаб розташований у місті","HQ is located in")} <b>${cityName(player.headquartersCity)}</b>. ${tr("Звідси можна оглядати стан будівель, але для будівництва, облаштування та роботи з NPC потрібно повернутися до штабу.","From here you can review the buildings, but to construct, furnish or work with NPCs you must return to HQ.")}</div>`:"";
  const art=`<div class="room-art"><img src="assets/hq/${r.image}.png" onerror="this.style.display='none'"><span class="art-placeholder">${tr("Зображення приміщення","Room image")}</span></div>`;
  if(!r.unlocked){
    document.getElementById("roomContent").innerHTML=remoteNotice+`<div class="room-layout"><div>${art}</div><div class="construction-card"><h2>${r.icon} ${roomName(r)}</h2><p>${roomDesc(r)}</p><p class="muted">${tr("Ця локація ще не належить штабу. Відкрийте її, щоб призначати NPC та облаштовувати предметами.","This location is not yet part of the HQ. Build it to assign NPCs and place furnishings.")}</p><button class="btn green" onclick="buildRoom('${r.key}')" ${atHeadquarters()?"":"disabled"}>${tr("Збудувати / відновити","Build / restore")} (${r.buildCost})</button></div></div>`;
    return;
  }
  const done=roomActionsUsed[r.key]===day;
  const staff=r.job?assignedStaff(r.job):[];
  const staffing=r.job?(staff.length?staff.map(person=>`<b>${htmlName(person)}</b> (${statusLabel(person)}, ${skillLabel(r.skill)} ${person[r.skill]})`).join("<br>"):tr("Ніхто не призначений. Приміщення не дає робочого ефекту.","No one is assigned. The room gives no work effect.")):tr("Призначення не потрібне.","No assignment required.");
  const earning=["forge","weaving","jewelry","furniture","stable","house","inn"].includes(r.key)?`<span class="badge">${tr("Дохід","Income")}: +${dailyRoomIncome(r.key)} / ${tr("день","day")}</span>`:"";
  const furnishSlots=Array.from({length:5},(_,index)=>{
    const item=homeItemById((r.furnishings||[])[index]);
    return `<div class="furniture-slot">${item?`${item.icon}<b class="quality-name-${itemRarity(item).colorClass}">${itemName(item)}</b>${rarityBadge(item)}<button class="btn gray" onclick="removeFurniture('${r.key}',${index})">${tr("Зняти","Remove")}</button>`:`<span class="muted">${tr("Вільний слот","Empty slot")}</span>`}</div>`;
  }).join("");
  const suitable=homeItemCatalog.filter(item=>homeStock(item.id)>0 && item.rooms.includes(r.key)).map(item=>`<button class="btn green quality-button-${itemRarity(item).colorClass}" onclick="furnishRoom('${r.key}','${item.id}')">${itemName(item)}</button>`).join("")||`<span class="muted">${tr("У запасі немає доречних предметів для цього приміщення.","No suitable items in storage for this room.")}</span>`;
  const homeRepairs=r.key==="house"?`<div class="workshop-actions"><b>${tr("Відновлення дому","House restoration")}</b><div class="trade-actions">${r.kitchenRestored?`<span class="badge effect-positive">${tr("Кухню відновлено","Kitchen restored")}</span>`:`<button class="btn green" onclick="restoreHousePart('kitchen')">${tr("Відновити кухню","Restore kitchen")} (140)</button>`}${r.familyWingRestored?`<span class="badge effect-positive">${tr("Сімейні покої відновлено","Family wing restored")}</span>`:`<button class="btn green" onclick="restoreHousePart('family')">${tr("Відновити покої родини","Restore family wing")} (220)</button>`}</div><div class="house-rooms"><b>${tr("Кімнати звичайних NPC","Rooms for ordinary NPCs")}</b><span class="badge">${tr("Відкрито","Open")}: ${r.freeRooms||0} / ${BALANCE.maxHouseRooms}</span><span class="badge">${tr("Зайнято","Occupied")}: ${unassignedHouseResidents().length}</span><span class="badge">${tr("Вільно","Free")}: ${freeHouseRooms()}</span><div class="trade-actions"><button class="btn green" onclick="buildHouseRoom()" ${atHeadquarters()&&((r.freeRooms||0)<BALANCE.maxHouseRooms)?"":"disabled"}>${tr("Відкрити кімнату","Open a room")} (${nextHouseRoomCost()})</button></div><p class="muted">${tr("NPC без роботи живуть у звичайних кімнатах. Родинні покої для чоловіків, дружин і дітей показані нижче окремим блоком.","Idle NPCs live in ordinary rooms. The family wing for spouses and children is shown separately below.")}</p></div></div>`:"";
  const production=r.key==="forge"?`<div class="workshop-actions"><b>${tr("Виробництво коваля","Smith production")}</b><div class="trade-actions"><button class="btn blue" onclick="craftWorkshop('forge','Інструменти')">${tr("Інструменти: 1 залізо","Tools: 1 iron")}</button><button class="btn blue" onclick="craftWorkshop('forge','Зброя')">${tr("Зброя: 2 заліза","Weapons: 2 iron")}</button><button class="btn blue" onclick="craftWorkshop('forge','Обладунки')">${tr("Обладунки: 3 заліза + шкіра","Armour: 3 iron + leather")}</button></div></div>`:r.key==="weaving"?`<div class="workshop-actions"><b>${tr("Виробництво ткача","Weaver production")}</b><div class="trade-actions"><button class="btn blue" onclick="craftWorkshop('weaving','Килими')">${tr("Килими: 2 вовни/льону","Carpets: 2 wool/flax")}</button><button class="btn blue" onclick="craftWorkshop('weaving','Одяг')">${tr("Одяг: 2 льону","Clothing: 2 flax")}</button><button class="btn blue" onclick="craftWorkshop('weaving','Сумки')">${tr("Сумки: 2 шкіри","Bags: 2 leather")}</button><button class="btn blue" onclick="craftWorkshop('weaving','Шапки')">${tr("Шапки: 1 вовна","Caps: 1 wool")}</button></div></div>`:r.key==="jewelry"?`<div class="workshop-actions"><b>${tr("Виробництво ювеліра","Jeweller production")}</b><div class="trade-actions"><button class="btn blue" onclick="craftWorkshop('jewelry','Прикраси')">${tr("Прикраси: срібло або 2 скла","Jewellery: silver or 2 glass")}</button></div></div>`:r.key==="furniture"?`<div class="workshop-actions"><b>${tr("Виробництво столяра","Carpenter production")}</b><div class="trade-actions"><button class="btn blue" onclick="craftWorkshop('furniture','Меблі')">${tr("Меблі: інструменти + шкіра","Furniture: tools + leather")}</button></div></div>`:"";
  document.getElementById("roomContent").innerHTML=remoteNotice+`<div class="room-layout"><div>${art}<div class="card room-detail"><div class="card-title"><b>${r.icon} ${roomName(r)}</b><span class="badge">${tr("Рівень","Level")} ${r.level}</span></div><p>${roomDesc(r)}</p><p class="muted">${roomEffect(r)}</p><span class="badge">${roomStat(r)}: +${r.bonus*r.level}</span>${earning}<span class="badge">${tr("Покращення","Upgrade")}: ${upgradeCost(r)} ${tr("монет","coins")}</span>${homeRepairs}${production}</div></div><div class="card"><h2>${tr("Облаштування","Furnishings")}</h2><p class="muted">${tr("До п'яти доречних предметів. Меблі однієї функції замінюють попередні.","Up to five fitting items. Furniture of the same function replaces the previous one.")}</p><div class="furniture-grid">${furnishSlots}</div><div class="trade-actions">${suitable}</div><div class="staff-list"><b>${tr("Призначені люди","Assigned people")}:</b><br>${staffing}</div><div class="trade-actions"><button class="btn" onclick="upgradeRoom(${currentRoom})" ${atHeadquarters()?"":"disabled"}>${tr("Покращити","Upgrade")}</button><button class="btn blue" onclick="useRoom('${r.key}')" ${done||!atHeadquarters()?"disabled":""}>${done?tr("Вже виконано сьогодні","Already done today"):roomAction(r)+" ("+r.cost+")"}</button></div></div></div>`;
}
function renderFamilyRooms(){
  ...
  const privateRooms=privateRoomPeople.length?privateRoomPeople.map(person=>{
    const slots=Array.from({length:5},(_,index)=>{
      ...
      return `<div class="furniture-slot">${item?`${item.icon}<b class="quality-name-${itemRarity(item).colorClass}">${itemName(item)}</b><button class="btn gray" onclick="removePrivateFurniture(${person.id},${index})">${tr("Зняти","Remove")}</button>`:`<span class="muted">${tr("Вільний слот","Empty slot")}</span>`}</div>`;
    }).join("");
    const choices=homeItemCatalog.filter(item=>homeStock(item.id)>0&&suitableForPrivateRoom(item)).map(item=>`<button class="btn green" onclick="furnishPrivateRoomItem(${person.id},'${item.id}')">${itemName(item)}</button>`).join("")||`<span class="muted">${tr("Потрібні доречні меблі з крамниці.","Need suitable furniture from the shop.")}</span>`;
    return `<div class="family-room private-furnished-room"><b>${htmlName(person)}</b><span class="badge">${person.spouse?tr("Подружня кімната","Marital room"):tr("Підготовлена кімната","Prepared room")}</span><span class="muted">${person.spouse?tr("Приватний простір родини.","Private family space."):tr("Кімната чекає на рішення про шлюб.","The room awaits a marriage decision.")}</span><div class="furniture-grid">${slots}</div><div class="trade-actions">${choices}</div></div>`;
  }).join(""):`<div class="empty">${tr("Приватних кімнат ще не облаштовано.","No private rooms have been furnished yet.")}</div>`;
  const familyChildren=children.length?children.map(child=>`<div class="family-room"><b>${htmlName(child)}</b><span class="badge">${tr("Вік","Age")}: ${child.age}</span><span class="muted">${tr("Мати","Mother")}: ${escapeHtml(child.childOf.motherName)}. ${child.status==="child"?tr("До повноліття не працює.","Does not work until of age."):tr("Повнолітній член родини.","An adult family member.")}</span></div>`).join(""):`<div class="empty">${tr("Дітей у родині поки немає.","No children in the family yet.")}</div>`;
