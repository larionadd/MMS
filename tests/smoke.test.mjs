...
assert.match(source, /function html5TravelMapBackdrop/, "travel map has a reusable backdrop helper");
assert.match(source, /assets\/map\/europe_1205\.png/, "travel map uses the raster Europe map as its backdrop");
assert.match(source, /const cityMapPoints/, "travel map city markers use calibrated image-overlay coordinates");
assert.match(fs.readFileSync(path.join(root, "style.css"), "utf8"), /\.travel-map-image/, "raster travel map image has dedicated styling");
...
assert.match(game.nodes.travelMap.innerHTML, /travel-map-image/, "travel map renders the raster map image under the interactive overlay");
